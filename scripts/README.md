...
- `run_suite.py`: expand a very small config-driven pilot matrix, launch sequential M4 runtime runs, then reuse the M5 summary path to emit canonical run summaries plus a config-selected summary file such as `pilot_summary.{json,md}` or `block_a_pilot_summary.{json,md}` and `run_plan.json`
