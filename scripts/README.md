...
- `summarize_block_a_final_closure.py`: merge the existing Block A master checkpoint with the runtime-only ablation, prompt-only ablation, and harder-manipulation slice, then emit `block_a_final_closure_summary.{json,csv,md}` plus canonical merged `run_summary.{jsonl,csv}`, `aggregate.json`, and `validation.json`
- `package_block_a_paper.py`: read the canonical `block_a_master_summary.json`, export paper-facing CSV tables under `paper_tables/`, render dependency-free PNG heatmaps under `paper_figures/`, and write `analysis/block_a_analysis.md`
