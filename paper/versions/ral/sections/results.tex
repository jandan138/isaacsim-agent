...
across both task families and persists as tasks become harder.

\input{figures/main_condition_ordering.tex}
\input{tables/main_outcome_summary.tex}

Table~\ref{tab:main-outcome-summary} records the same retained cohorts
in a reviewer-facing layout that keeps success, invalid-action exposure, and
...
\texttt{P1}/\texttt{P2} cells, so the repeated 1.0 success values remain
interpretable at manuscript scale.
...
overhead across the retained \texttt{P1} versus \texttt{P2} comparisons.
...
table. Figure~\ref{fig:planner-tool-overhead} compresses those retained
...
evidence of a new success regime.
