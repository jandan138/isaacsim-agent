...
each slice, P0/R0 is the weakest cell: it fails on all tasks (0/11 navigation
easy, 0/3 manipulation easy, 0/4 harder navigation, 0/3 harder manipulation),
with every run producing an invalid action. Pairing the same under-specified
contract with R1 changes the outcome: P0/R1 succeeds on all tasks (11/11,
3/3, 4/4, 3/3) through recovery after one retry per run. P1/R0, P1/R1,
P2/R0, and P2/R1 all succeed on every task with zero invalid actions.
...
structured conditions, P2 matches P1 on success while showing fewer
planner/tool calls across the evaluated \texttt{P1} versus \texttt{P2}
comparisons.
...
evidence of a separate success mechanism. Note that planner/tool call counts
are a discrete workload proxy, not a latency, token-cost, or wall-clock
measurement.

\input{figures/planner_tool_overhead.tex}

\input{tables/planner_tool_overhead_summary.tex}

\subsection{Action-Interface and Runtime-Only Ablations}

The targeted ablations use different easy-task subsets: the runtime-only
\emph{repair-probe cohort} deliberately includes recoverable
invalid-first-action probes under fixed P1, whereas the action-interface
ablation uses clean easy tasks under fixed R0. The repair-probe cohort was pre-specified from the easy-task pool
using a single inclusion criterion: tasks known to elicit a recoverable
invalid-first-action emission under P1/R0. This yielded four tasks in
total: two navigation and two tabletop-manipulation tasks. Together they isolate the two
design axes within the same execution-design framing.
...
This slice is intentionally constructed to stress-test recoverable
invalid-first-action cases; the 2/4 to 4/4 result should be interpreted as
evidence that R1 can repair this specific failure class, not as a general
runtime benefit estimate.
...
The failure traces identify the dominant P0 failure mode. In navigation,
failing P0 runs emit \texttt{move\_to\_goal}; in tabletop manipulation, they
emit \texttt{move\_object}. Both are semantically plausible task verbs, but
the executor only dispatches \texttt{navigate\_to} and
\texttt{scripted\_pick\_place\_step}. As described in Section~III-C, these
are injected boundary probes rather than stochastic foundation-model errors.
R1 helps because the runtime parses the JSON, validates the tool name and
required arguments, and returns the literal error string before one retry.
The retry therefore does not introduce a richer second-pass schema; it turns
a terminating handoff failure into a recoverable one.
...
\paragraph{Isaac Sim cross-backend check.}
Two navigation instances were also evaluated under the minimal Isaac Sim
stage. The same ordering holds: P0/R0 fails, P0/R1 recovers, P1/P2 succeed
with zero invalid actions. This provides a limited cross-backend check that the same boundary
ordering also appears in the physics-backed slice.

\subsection{Harder-Task Robustness}

The harder-task slices increase planner/tool call counts without changing the
