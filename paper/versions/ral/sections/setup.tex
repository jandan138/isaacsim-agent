...
summaries, this appears as \texttt{planner\_backend=mock\_block\_a} with
\texttt{model\_variant=null}; the implementation is the local file
...
stochastic decoding settings to tune in this frozen study.
...
\texttt{self\_check} field. R0 performs bare dispatch, while R1 validates tool
name and arguments against the active task-specific namespace and allows one
repair attempt before termination. Navigation exposes
\texttt{get\_robot\_state}, \texttt{get\_goal\_state}, and
\texttt{navigate\_to}. Manipulation exposes \texttt{get\_gripper\_state},
\texttt{get\_object\_state}, \texttt{get\_target\_state}, and
\texttt{scripted\_pick\_place\_step}. The archived task configs, trajectories,
and processed summaries record the selected backend realization, contract and
runtime variants, termination reason, and Isaac runtime metadata such as
\texttt{backend\_variant} and \texttt{kit\_version}=106.5.0, so the reported
slices remain traceable to fixed local artifacts.
...
The paper draws its quantitative comparisons from the frozen main factorial
results plus targeted runtime-only, action-interface ablation, and harder-task
summaries.
