...
2. Runtime validation plus a single retry has independent value when prompt structure is fixed at `P1`.
...
4. Prompt/interface structure has independent value when runtime is fixed at `R0`.
...
5. `P2` is more planner/tool-efficient than `P1` in the tested prompt-only and harder-manipulation slices while keeping the same success profile.
   Support:
...
1. The current evidence suggests that interface structure matters primarily by reducing invalid actions.
   Reason:
   this is an interpretation of the prompt-only ablation and the cross-family pattern, not a separately manipulated mechanism study.
2. The current evidence suggests that lightweight runtime support is most valuable for fragile executions that are still recoverable.
   Reason:
   the runtime-only slice was deliberately built around recoverable invalid-first-action probes.
3. The brief self-check in `P2` appears to reduce planner/tool overhead without harming success in the tested setup.
   Reason:
   the effect is stable in current slices, but the dataset is small and controlled.
4. Harder tasks currently look like cost amplification rather than rank reversal.
   Reason:
...
5. Block A is closed enough to support a controlled systems paper.
   Reason:
...
4. The paper does not claim statistical significance, seed robustness, or population-level inference.
5. The paper does not claim memory, context compression, tool abstraction, safety, or randomization findings.
6. The paper does not claim sim-to-real readiness or real-world safety.
7. The paper does not claim that the merged overall success rate of the final closure package is the main scientific result.
